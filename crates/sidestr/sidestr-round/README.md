...
- A signature is journalled before it is handed out for publishing;
  `FileJournal` is append-only and `fsync`ed; on restart the one-signature
  rule applies against it.
- `resign_after`: `Some(propose_after)` is upstream's relaxation; `None`
  never re-signs a height.
...
Not yet: a signer on another machine, changing the signer set.
