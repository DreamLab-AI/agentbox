...
mod tests {
    ...
    fn addresses_round_trip_and_tolerate_colons_in_the_identifier() {
        let a = address(38_100, "ab".repeat(32).as_str(), "id:with:colons");
        let (kind, pk, d) = parse_address(&a).unwrap();
        assert_eq!(kind, 38_100);
        assert_eq!(pk.len(), 64);
    ...
    fn a_malformed_address_is_refused_rather_than_half_read() {
        assert_eq!(parse_address("38100"), None);
        assert_eq!(parse_address("38100:onlypubkey"), None);
