 ...
 *
 * Relocated here from mcp/servers/ by ADR-2108: it was never an MCP server, only
 * a pure request-descriptor builder that the retired `ontology-bridge` happened
 * to call first. The management API's KG-elevation scan is now its only caller,
 * so it lives inside that package's own boundary.
 *
 ...
 * human approval → PR. The retired bridge POSTed axioms straight to
 * `/api/ontology/load`, bypassing every gate. This module:
 *
 *   - builds the `/api/ontology-agent/propose` request body (the former `ontology_propose`
 *     descriptor) mirroring VisionClaw's ProposeRequest contract, defaulting the
 *     agent context from the agent's did:nostr environment; and
 *   - guards the ungoverned `/api/ontology/load` backdoor behind
 *     `AGENTBOX_ONTOLOGY_DIRECT_LOAD` (default off) so a direct-load caller
 *     refuses by default and is pointed at the governed descriptor.
 *
 ...
 *
 * @see PRD-014 §4.4  @see ADR-2108  @see src/handlers/ontology_agent_handler.rs (propose)
 * @see `vault propose` (contract C2) — the corpus-side successor WS-F wires in.
 */
