#!/usr/bin/env bash
# ============================================================================
# Agent-of-Empires command-override wrapper — OpenRouter-routed Claude Code
# ----------------------------------------------------------------------------
...
#
#   * HOME and CLAUDE_CONFIG_DIR are BOTH pinned at $WORKSPACE/profiles/openrouter
#     so Claude Code reads that profile's runtime-written settings.local.json
#     (its own ANTHROPIC_BASE_URL + token) and never the global ~/.claude that
...
[ -f "$SETTINGS" ] || _die \
  ...
  "the redirect key was never provisioned — set OPENROUTER_API_KEY in .env" \
  "and reboot so the reconciler writes the profile settings."
