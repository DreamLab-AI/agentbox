...
status: accepted (realised — WS5 producer convergence + ADR-059 Phase-1 mirror done 2026-05-29; WS5-D1 agent-action render + ACSP 31402 dispatcher (WS7) landed in the VisionClaw (/project) slice, verified 2026-08-21). Seam E is BUILT: BC20 `urn:agentbox` ingest is live (`project/src/agent_events/ingest.rs:325`, plus ~10 further `project/src` modules referencing `urn:agentbox`); ACSP kinds 31400-31404 are defined (`project/src/services/acsp/events.rs:18-23`) with a working 31402 ActionRequest dispatcher (`project/src/actors/decision_elevation_actor.rs`, `project/src/services/voice_intent_client.rs:281`); and the `/wss/agent-events` consume side is wired (`project/src/actors/agent_beam_actor.rs:68`). Commits `e7771eb90` / `43a12e401`. The one residual is agentbox-side: WS4 pod-mandate enforcement is not fail-closed (uris.js mandate kind exists but pod NIP-98 originator is opt-in — _solid-http-base.js falls back unsigned).
date: 2026-05-28
type: integration
...
- **Seam E — elevation → visualisation (VisionClaw + BC20). BUILT (verified
  2026-08-21) — the VisionClaw (/project) slice landed the render + dispatcher.**
  The BC20 anti-corruption layer is no longer paper-only: `urn:agentbox` ingest
  is live at `project/src/agent_events/ingest.rs:325`, with ~10 further
  `project/src` modules (`uri/mod.rs`, `services/acsp/events.rs`,
  `services/proposal_spine.rs`, `services/decision_elevation.rs`,
  `services/provenance_writer.rs`, `agent_events/schema.rs`, …) referencing the
  grammar. ACSP is spoken: kinds 31400-31404 are defined
  (`project/src/services/acsp/events.rs:18-23`) and the ACSP 31402 ActionRequest
  dispatcher (WS7) exists (`project/src/actors/decision_elevation_actor.rs`,
  `project/src/services/voice_intent_client.rs:281`), with agent-action render
  fed over the `/wss/agent-events` consume side
  (`project/src/actors/agent_beam_actor.rs:68`, `encode_agent_actions`). The
  server binary protocol defines the `0x23 AGENT_ACTION` frame and the
  `agent-action` WS event (`project/src/utils/binary_protocol.rs:1187-1463`).
  Commits `e7771eb90` / `43a12e401`. WS5-D1 render + WS7 dispatcher are done; the
  agentbox producer side (BC20 bridge, propose surfaces) was already done.
