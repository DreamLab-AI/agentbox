...
- **F4-1.** The **only** ingress to `:9095` is a NIP-98-verifying reverse proxy — extend the existing https-bridge (`:3001`) or add a sibling thin proxy reusing `management-api/middleware/auth.js` verification, forwarding to the daemon with `X-Forwarded-For`. `aoe serve` runs `--behind-proxy --auth token` on loopback (D4.6). **Revised 2026-08-31:** the daemon moved from `--auth none` to `--auth token` — loopback is no longer the boundary. The daemon mints a shared-secret token at launch (`~/.config/agent-of-empires/serve.url`); the proxy reads it and injects `Authorization: Bearer` upstream (the proxy stays the sole *identity* ingress). The break-glass `nostr-gateway` reads the same token file rather than an unauthenticated direct route.
...
- **F4-3.** **Invariant:** nothing other than the proxy may reach `:9095` **as an identity ingress**. `--behind-proxy` trusts `X-Forwarded-For`, so the daemon MUST bind `127.0.0.1` and the proxy MUST be the sole identity ingress, or any container-local process reaching the port bypasses NIP-98 identity (D8; NFR N-05, invariant). **Revised 2026-08-31:** loopback binding alone was insufficient — a co-resident process could reach `:9095` tokenless. The daemon now runs `--auth token`, so every request must also carry the daemon's shared-secret token; a process that cannot read the token file (`serve.url`, dir 0700) cannot drive the daemon. Residual limit: same-uid (devuser) processes can still read the token file — token + loopback raises the bar but does not isolate same-user peers (per-process isolation is future work).
