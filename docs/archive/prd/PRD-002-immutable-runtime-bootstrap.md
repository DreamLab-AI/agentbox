...
7. For `playwright` browsers: the `flake.nix` already includes `pkgs.playwright-driver` in `browserPackages`. Set `PLAYWRIGHT_BROWSERS_PATH=${pkgs.playwright-driver.browsers}` in `imageEnv` when `browserCfg.playwright` is true. Remove the `npx playwright install chromium` call from Phase 7. **Superseded: local Playwright browser packaging is no longer the automation path — see the `browsercontainer` sidecar. Item kept for historical rationale.**
