...
| ↳ P1 status 2026-09-22 | **Minted.** `sidestr:dreamlab` sealed beside `tbtc4` by the merged upstream engine (spec `53f91f9`): genesis `4db37517…d453dbc0`, signer `7092810a…4c76d62`, prefix `drm`, no pegs; document at `config/sidechain/dreamlab/chain.json`, key in the secrets volume, block file under `$WORKSPACE/sidestr/dreamlab`. Replayed cold by the engine; block 0's header hashed independently to the document (`tests/config/sidechain-genesis.test.sh`). The testnet4 peg wallet holds 0.001 tBTC. **Live since 12:24 UTC:** the upstream JS producer runs as an interim tmux program (`config/sidechain/run-producer.sh`), announcing kind 33333 to the five default relays and naming the GitHub Pages mirror `dreamlab-ai.github.io/sidestr-dreamlab` (`mirror-sync.sh`); first announcement reached 5/5 relays. | Not yet: supervised programs, `:9097` mirror behind the proxy, URN kinds, 38420 binding, first peg-in. Upstream gap found on listing: `parseTip` accepted only 164-byte v2 headers, so stock-family chains were invisible to the directory, explorer and wallet; fix filed as sidestr/spec PR #7, **merged 2026-09-22 18:59 UTC** and shipped in `@sidestr/spec` 0.0.3 / client `sidestr` 0.0.4: `npx sidestr info --chain sidestr:dreamlab` finds the chain from the relays alone and judges the Pages mirror against the announcement (ok at height 33); the parser lives in one upstream file, so no wallet or explorer pin bump was needed. Level 2 usable now: `sidestr-round` 0.1.0 (`cosign`) co-signs with the JS signers live. | Deviation found: upstream's genesis commits id, pegs, time and signer only, so `parent` and the containment block are bound by the committed document, not the coinbase (ADR-2103 amendment). |
