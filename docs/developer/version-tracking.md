...
| Codex Rust CLI version | `lib/codex-binary.nix` (`codexVersion` + per-arch sha256) | `./agentbox.sh update` resolves latest + both release hashes |
...
- **Custom regex pins** — ComfyUI, Gemini CLI, gitleaks-action version strings.
...
```sh
./agentbox.sh update
# Codex/npm CLIs without updating flake inputs:
./agentbox.sh update --cli-only
```

The checked-in version and hashes remain the reproducible build record; they are
generated by the update sequence rather than maintained by hand. Review the
diff, let `flake-check.yml` pass, then merge.

Manual hash rotation remains the pattern for ComfyUI (`comfyuiHash` in
`flake.nix`) and LichtFeld (`lichtfeldSha256` in `lib/3dgs-stack.nix`).
