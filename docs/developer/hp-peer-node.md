...
| Image | `agentbox:runtime-x86_64-linux`, streamed from the ml daemon: `docker save agentbox:runtime-x86_64-linux \| ssh john docker load` (41.8 GB, roughly 25 MB/s, under half an hour). No Nix build on the connected node |
...
| Env | `.env` carries `RUVECTOR_PG_PASSWORD` only. No API keys are copied to the connected node |

`docker-compose.override.yml` must not be used on the connected node. It is the gateway host-specific: the MAD
...
## Manifest deltas (`agentbox.toml` on the connected node)
...
| `[networking]` | `tailscale = false`, `hostname = "agentbox-hp"`, and delete `[security.exceptions.tailscale]` | No auth key on the connected node; the validator (E020) rejects an exception block for a disabled gate |
...
|---|---|---|
| `:9096` NIP-98 door, unsigned | ml to the connected node | 401 |
| `:9096` NIP-98 door, signed by the ml node key | ml to the connected node | 200, `X-Agentbox-Pubkey` stamped |
| `:9096` NIP-98 door, signed by the connected node node key | the connected node to ml | 401 `pubkey_not_allowed` until the connected node key was added to the ml proxy allowlist, then 200 |
| Embedded relay `:7777`, allowlisted signer | ml to the connected node | `OK true`, event stored and readable back |
| Embedded relay `:7777`, ml node key (not in the baked allowlist) | ml to the connected node | relay `OK true`, `nostr-pod-bridge` logs `rejected pubkey=...` and drops it |
...
To reach the connected node relay from the ml side for testing, run a tap inside the connected node container and tunnel
...
Kill the tap afterwards; it exposes the relay on the connected node docker bridge.

## Admitting the connected node node on the ml side

Add the connected node `agentbox-core` x-only pubkey to `[interaction_plane.proxy].allowed_pubkeys` in the ml
...
* The connected node GPUs are close to full with the Loom model container. The agentbox has passthrough but
  little free VRAM.
* `docker cp` into the container fails (`read_only` rootfs). Use
  `docker exec -i agentbox sh -c 'cat > /tmp/file' < file`; `/tmp` is a tmpfs.
* The connected node node does not mount the estate checkout, so `workspace/project` is empty there. Tools
