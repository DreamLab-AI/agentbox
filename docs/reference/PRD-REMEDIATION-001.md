...
| **R-005 / SEC-001** | Runtime privilege-escalation path: setuid sudo, SETUID/SETGID caps, redundant runtime `chown`. | **No runtime privilege escalation.** SETUID/SETGID removed from `cap_add` (flake `baselineCapAdd` + compose); `no-new-privileges:true` kept; redundant runtime `sudo chown` calls removed. Root only at boot; comments corrected. | `3eee9b37`, `0bfa856c` |
...
4. **No runtime privilege escalation.** `docker inspect` shows `HostConfig.CapAdd` contains neither `SETUID` nor `SETGID`, `SecurityOpt` contains `no-new-privileges:true`; no setuid sudo on `PATH` for `devuser`. **Test:** `RM-001-04`.
