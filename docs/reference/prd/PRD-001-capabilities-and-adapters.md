...
- **Mobile agent bridge** — pure-Nostr phone↔agent path (replaced the retired Telegram/CTM mirror). An Android Nostr client (Amethyst + Amber signer) holds its own key plus a NIP-26 delegation from `[sovereign_mesh.operator]`; inbound NIP-59 gift wraps (kind 1059) are unwrapped by `services/nostr-pod-bridge` and persisted to the pod inbox, with a kind-30840 session-summary dual-written to relay + pod as the durable record. See [docs/user/mobile-bridge.md](../../user/mobile-bridge.md).
