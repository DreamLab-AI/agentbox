...
14. `E014` — **retired 2026-06-02.** Was: `[sovereign_mesh].telegram_mirror = true` requires `CTM_BOT_TOKEN` and `CTM_TELEGRAM_CHAT_ID` env vars at boot. The Telegram/CTM mirror was removed entirely; the phone↔agent path is now pure Nostr (Amethyst + Amber → embedded relay → `services/nostr-pod-bridge` → pod). No env-var precondition replaces it — the bridge keys are validated under the relay rules (E028/E030). Schema property dropped, validator rule removed.
15. `E015` — **retired 2026-04-25.** Was `[sovereign_mesh].jss_rust_backend = true` requires `jss-rust` Nix input pinned in `flake.lock`. The flake input was never declared and the field had no consumer; the Rust pod adoption shipped as `solid-pod-rs` (ADR-010) instead, and the JSS Rust crate work (did:nostr, NIP-98 Schnorr, webhook signing, rate-limit, quota, v0.4 compat) was absorbed into solid-pod-rs as default-on Cargo features. **No capability was lost.** Schema property dropped, validator rule removed, code reserved for re-use.
