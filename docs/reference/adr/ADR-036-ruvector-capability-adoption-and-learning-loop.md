...
## Amendment (2026-07-22) — review_trigger FIRED; ADOPT-LATER set resolved (read ADR-040)

This ADR's `review_trigger` has **fired**. [ADR-040](ADR-040-learning-consumers-model-lifecycle-and-legacy-mining.md) / [PRD-020](../prd/PRD-020-ruvector-learning-consumers-and-corpus-uplift.md) trip two of the named conditions (a ninth capability-adoption decision — the graph/ontology backbone; and promotion of ADOPT-LATER capabilities requiring the recall harness) and record the further facts the 2026-07-21 live audit surfaced. **Read ADR-040 as the current record.** The D1–D8 decisions below stand **as the v1 design**, but three statements in the TL;DR and Context are superseded — treat them as history, not current state:

- **The learning loop is now closed for capture→distil (no longer "severed").** D1 specified a producer *and* an aggregator; only the producer shipped in v1, so through 2026-07-21 the loop stayed open at the distillation stage (`memory-learning-aggregates` = 0 rows; no aggregator existed in the repo). ADR-040 D1 / PRD-020 W-A built the missing Wilson/recency sweep: the corpus is now **405 trajectories / 8,806 steps, all judged** (386 success / 19 failed / 0 unjudged), the aggregator holds **12 aggregates past the Wilson floor from 8,839 steps**, and distillation wrote **13 `judge:trajectory` patterns** into the `patterns` table (its first machine-distilled rows). Only `feed_retrieval` / `feed_routing` remain the next gates.
- **The ADOPT-LATER set (D4) is resolved — see the ADR-040 re-evaluation table.** Attention re-rank → **CLOSED-NO-BENEFIT** (the corpus is exactly L2-normalised, so `attention_score` = cos/√dim with max deviation 4e-7 — a mathematical no-op; `attention_rerank` is OFF by measurement, not caution). SONA → **BLOCKED-UPSTREAM** (the engine hardcodes `embedding_dim=256`; 384-dim learns are accepted-but-discarded — `sona_enabled` is split into `sona_learn_enabled` / `sona_apply_enabled`, both OFF until an image with configurable dim). Relevance-feedback → **re-scoped** as an HNSW `ef_search`/`probes` parameter auto-tuner (`param_tuning`, previously conflated with SONA), kept reserved.
- **The recall-regression harness the review trigger named now exists** — `./agentbox.sh ruvector recall`, frozen band **self ≥175/200 · true ≥107/120**. The v1 figures quoted below (self-recall@10 **188/200**, true recall@10 **119/120**) were a **pre-ingest** measurement and are **not** the current band. The **~1.84M legacy rows** the audit names as bloating the HNSW index were **exported to an 11G cold archive** (`VACUUM FULL` took `memory_entries` 34 GB → 614 MB; live store is now **~178,427 rows / 454 namespaces**), then W-E audited that archive in isolation — outcome labels 100% degenerate, zero output text — so **nothing was imported**.

**New field ops law** recorded by ADR-040 and now binding on this store: rebuild the HNSW index **non-concurrently** (`m=16`, `ef_construction=128`) after bulk ingests/deletions; **never `CREATE INDEX CONCURRENTLY` on the ruvector HNSW access method** (verified double-insertion). Durable supervisord scheduling for the sweep/distill loops is staged in flake.nix but not yet imaged — until the next rebuild they run as operator-started detached loops.
