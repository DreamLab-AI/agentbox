...
# ADR-2109 — Ontology proposals and decisions both go through `vault`

> Renumbered from ADR-2106 on 2026-09-22: agentbox ADR-2106 was already taken by the sidestr licensing ADR (committed 11:50 the same day).
...
3. **The blast radius is always declared and never widened.** Every edit carries `--expect docs=1,blocks=<n>`, where `n` is the number of distinct frontmatter keys that **actually change** — never a fixed number. `vault edit` counts a block per changed key, so the key set is planned against the page's current `status` (read, read-only, from `<VAULT_REPO>/knowledge/pages/<id>.md`) and `--expect` is derived from the very `--set` argv sent: promote draft→stable is `blocks=2` (`status`, `verified`); promote of an already-`stable` page is a **re-attestation** — `status` untouched, one `verified` appended, `blocks=1`; demote is `blocks=1`; a demote of an already-`deprecated` page sets nothing, spawns no edit, and is still ledgered. If the page changes between the read and the edit, the guard no longer matches and the vault refuses: the race fails closed. (Amended 2026-09-22: the original fixed `blocks=1` was refused by the real `vault` for every promote.) Every `vault` call is pinned with `--repo <root>`: `VAULT_REPO`, else derived from `VAULT_ROOT` (its parent when it ends in `knowledge/` or `working/`), and the root must hold `ontology/vocabulary.yaml` — otherwise nothing is spawned. `vault edit` refuses without it. If the vault disagrees, the edit fails and nothing is written — the correct outcome for a decision whose subject moved underneath it.
...
- The `--expect` guard is derived, not configurable (`docs=1`, `blocks` = keys set). A multi-page decision would need a new outcome, which is the right place for that conversation.
...
### Re-verified 2026-09-22 against the real `vault` (amendment)

- `node --test management-api/tests/{ontology-apply,ontology-propose-vault,elevation-stage}.test.js` — 73 pass (incl. the `kind: create` branch): repo resolution and fail-closed; `--expect` derived per current status (draft/stable/deprecated × promote/demote); re-attestation at `blocks=1`; no-op demote spawning no edit; canonical `sha256:` digest; the real `{proposal, event}` output unwrapped so blockers still block; the elevation staging gate.
- `npx jest` (management-api) — 88 suites, 1462 pass.
- `scripts/e2e-ontology-promotion.sh` — stub mode 30/30. `E2E_REAL_VAULT=1`: a scratch repo (never the live corpus) is seeded with a validated draft page; the real `vault propose` builds content and schema 31402s whose signatures verify and whose tiers the relay rule computes (schema ⇒ High); human 31403s bound to the real case id are applied and the page READ BACK — Promote ⇒ `stable` + one `verified` (human, 31403 instant), re-Promote ⇒ a second `verified`, status unchanged, Reject ⇒ sha256-identical and vault never spawned, Demote ⇒ `deprecated` with no stamp; the real Loom `/loom/attest` holds five entries on one case id with the canonical digest and a sound chain.
- **Relay publishing is verified dry-run only.** Every `vault propose` in these tests runs with `--dry-run`: the signed 31402 is built and checked, but no relay runs in this container, so publication to a live relay and the relay's own projection of a vault-built event are not exercised here (the projection is exercised against the worker's SQL on SQLite with fixture events).
- **Create path (closed 2026-09-22, `vault create` landed).** Elevation stages the candidate's promoted page (always declaring `title`, which names the file; IRI relation targets resolved to the ids of the pages that answer to them, since the vault links by id — an unresolvable IRI is not proposable), validates it on its own, and `vault propose --diff` marks it `kind: create`. A human Promote of that 31402 (looked up by the 31403's `e` tag, accepted only for the same `d` case; absent/`amend` keeps the edit path) is applied from ONE source of truth — the page the SIGNED /dev/null diff adds (sha256 must equal `page_sha256` when present; IRI, page and `resource` checked) — by ONE write: `vault --repo <root> create <file> --expect docs=1 --set status=stable --set 'verified+={by: human:<npub>, at: <rfc3339>}' --json` (built only in `createArgsFor`). Every refusal exits 2 with `{created:false, file, code, message, blockers}` and writes nothing: `EXISTS` is `applied:false, reason:'exists'`; any other code (`BLOCKED`, `FRONTMATTER_INVALID`, `UNTITLED`, `UNDECLARED_BLAST_RADIUS`, `GUARD_VIOLATED`) throws with that code and its blockers; neither is ledgered. The attestation is one argv element, a YAML flow mapping with a space after each colon (`{by,at}` would be stored as a plain string); real mode asserts it lands as a `by`/`at` mapping. Real mode proves it end to end: the created page exists, is `stable` with one `verified` naming the human at the 31403 instant, passes `vault validate --strict`, is ledgered once under its own case, and a second approval is refused `EXISTS` with the page byte-identical. E2E real mode 101/101, stub mode 30/30.
