...
- **`agentic-qe` npm CLI (`aqe`) bumped `3.10.5` → `3.11.1`** in `flake.nix` (upstream [v3.11.1 release](https://github.com/proffesor-for-testing/agentic-qe/blob/main/docs/releases/v3.11.1.md)). Tarball `sha256` is pinned to the verified value (`sha256-Yrd6Dtj4fVQU9D3NXjYB65JLhopUs1Yi9XRnemkjS+0=`); `nodeModulesHash` is reset to `lib.fakeHash` because 3.11.1 adds/bumps dependencies (`@huggingface/transformers`, `vibium`, `axe-core`, `jose`, `secure-json-parse`, `prime-radiant-advanced-wasm`, `better-sqlite3@^12`) and the recursive node_modules NAR hash can only be realised on a networked Nix build host — resolve with `./scripts/prefetch-hashes.sh --cli`. New upstream capabilities (best-of-k + cross-model best-of-k, opt-in adversarial-verify/`verifyGate`, MCP self-governance via `.harness/mcp-policy.json`, cost-Pareto scoring, Ed25519 witnessed delivery, Goodhart guard, qwen3:30b-a3b default free-tier model) are all **opt-in and default-off**, so the install and `aqe init` contract is unchanged. The `aqe` binary name, the `agentic_qe` manifest gate, and the `agentic-qe-probe.sh` artifact probe are unaffected. The `build-with-quality` harness skill doc (v1.4.0) gains an "Agentic QE Upstream Capabilities (v3.11.1)" section mapping each new feature to the skill's existing evidence/anti-fox model.
