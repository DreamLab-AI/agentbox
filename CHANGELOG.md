...
### Changed (2026-09-05 capability gates, ADR-2057)

- **Closed the three capability-gate gaps that made `byte-identical-when-off` false.** New `[skills.podcast_ingest]` gate (default `true` — exactly the behaviour `[program:podcast-cron]` shipped with, so enabling is never a migration step) wraps the last unconditionally supervised program in `lib.optionalString`; the `harness` and `precedent` MCP registration blocks in the entrypoint now read `skills.<gate>.enabled` via `agentbox-manifest toml-bool` instead of checking only that the server file exists; and all three gained `system-manifest.js` catalogue entries with honest apply classes — `boot` for the two entrypoint registration gates, `rebuild` for the flake-baked supervisor block (catalogue now resolves 60 gate paths, was 57). **Behaviour change:** anyone who set `[skills.harness]` or `[skills.precedent]` `enabled = false` and did not notice it was ignored will now, on a fresh workspace, stop getting those MCP servers. Two documented residuals: gate-off skips registration but cannot retract an entry an earlier boot already wrote (no remove-by-name `agentbox-manifest` subcommand), and the podcast gate removes the schedule, not the shared `podcast-ingest`/`supercronic` derivations from the image closure.
- **`[skills.harness].template_dir` is no longer an inert manifest key.** `harness-bridge.js` has always read `HARNESS_TEMPLATE_DIR`; nothing exported it, so the server silently used its hardcoded default. The entrypoint now projects the manifest value into the server's `.mcp.json` env (falling back to that same default when unset, so an absent key changes nothing) and, like the ontology block, drops the `grep` guard so a `template_dir` change propagates on every boot instead of freezing at first registration.

### Changed (2026-09-05 governance gaps, ADR-2068..2071)

- **The root session now runs the session hooks its gate turns on** (ADR-2068). `[ontology_monitor].enabled` reached only per-profile sessions via `stacks.rs`; the root `~/.claude/settings.json` — tmux window 0 and every unattended teammate pane — had neither the `SessionEnd` entry nor the `AGENTBOX_ONTOLOGY_MONITOR` master switch that stops the hook being a no-op. `config/entrypoint-unified.sh` now seeds both from the manifest gate and, when the gate is off, **retracts** hook and env back to the pre-gate bytes (ADR-2020) rather than merely skipping — `settings.json` persists on the workspace volume, so skip-on-off left a live hook behind. Five fixture cases pass, one of which caught an empty `env: {}` leak on the off path.
- **`config/hooks/README.md` names which files are hooks and which are CLIs** (ADR-2068). Not everything in `config/hooks/` is registered on an event: `project-tracking-publish.cjs` is a CLI spawned by the management API from `POST /v1/projects/:id/publish`, and `fleet-tab-name.sh` is a helper shelled by `fleet-session-start.sh`. The table also splits root-session registrations (entrypoint) from per-profile ones (`stacks.rs`), which is the drift that hid the gap above.
- **An uncatalogued manifest gate key now fails the build** (ADR-2069). `scripts/ci/check-manifest-catalogue.js` printed 130 uncatalogued boolean keys as a WARN and exited 0, so a capability could ship without ever appearing in `/v1/system`. Uncatalogued keys now fail unless listed in an explicit BASELINE, and the BASELINE is a **ratchet**: an entry that has since been catalogued, or that no longer exists in `agentbox.toml`, fails too, so the list can only shrink. 17 keys sit in a group named UNCATALOGUED CAPABILITY and are printed every run — real gates still owed a catalogue entry, deliberately not laundered into "not a capability". Already wired in `invariants.yml`; no workflow change needed.
- **ADR-045's "one front door" is an ingress rule, so the Loom's raw port is not a breach of it** (ADR-2070). `GOVERNANCE-capabilities` divergence 3 applied an ingress invariant (`:9096`, NIP-98, control surfaces reaching *into* the box) to an *egress* choice. The raw `:8085` door stays — deliberate, named in `flake.nix` (`LOOM_RAW_BASE_URL`) and the `loom-raw` session seed — as agent-choice and benchmark-only for raw coding, explicitly not a fallback and never auto-routed when the façade errors. The model-swap contract (consumers hold `:8084`) is unchanged. Residual: documentary, unenforced at runtime.
- **The nightly dream cycle stays outside the execution journal, and now says why** (ADR-2071, **proposed**). Its SSH/LLM/`git push`/forum side effects classify as `egress`/`mutate` — in `APPROVAL_REQUIRED` — while `action-plane.js` wires no approver, so a faithful routing would deny the night on its first action; classifying the cycle `local` to keep it running would launder the gap. The record stages journalling (~380 lines, HTTP-only: `local-jsonl` caches the hash-chain head in memory and appends unlocked, so any second writer forks the chain) ahead of policing, and names a two-character precondition — the pipeline returns `decision: 'deny'` while `action-plane.js:276` and `routes/tasks.js:85` both test `'denied'`, so the first guard added turns an intended 403 into a 500.

### Changed (2026-09-05 federation kind-map parity, ADR-2061)

- **The cross-repo URN kind list is one versioned artefact both translators derive from, and asymmetry is now a test failure** (ADR-2061). The JS bridge crossed `bead` structurally (`bc20-provenance-bridge.js:167-182`) while VisionClaw's `cross_from_agentbox()` refused it at the wildcard, so the same object crossed in one language and was dropped in the other — two closed maps written from two lists, with nothing detecting the disagreement. New `schema/federation-kinds.json` (artefact 1.0.0, all 19 `uris.js` kinds) is the single authority: the bridge reads it at require time and builds `AGENTBOX_TO_VISIONCLAW` from the rows, and VisionClaw's `src/uri/mod.rs` embeds the **same bytes** with `include_str!("../../agentbox/schema/federation-kinds.json")` — the agentbox tree is that checkout's `./agentbox` submodule, so the relative path resolves — and dispatches on the artefact's `target_kind` instead of on a hard-coded kind. Neither side transcribes the list any more, and neither test hard-codes it: every case is generated from the artefact. Each row carries the source grammar, the target grammar, a crossing form, and a fixture; the fixture outputs were computed by the JS hasher and are reproduced **byte-for-byte** by Rust's independent sha256, extending the `entity_urn_matches_uris_js_golden` proof from the content address to the whole crossing. `memory` is a **recorded** refusal — `refusal_class: "deliberate"`, naming the `{domain, slug}` elevation that would unlock it and asserted to still cross when those are supplied — so "deliberately closed" is legible as something other than "not implemented", and every other refusal is `not-federated` with its own reason. Gate proven by deliberately flipping `memory` to `crosses: true`: 3 of 50 jest cases and 4 of 7 `cargo test` cases failed, naming the kind and the missing arm. Residual: only the JS half runs in CI — `contract-tests.yml` already globs `tests/contract/*.contract.spec.js`, but VisionClaw's `CPU_CRATES` excludes the `visionclaw-server` crate that owns `src/uri`, so the Rust fixture is green locally and needs a one-line `ci.yml` addition from the owning lead.

### Fixed (2026-09-05 sovereign data path, ADR-2064/2065)

- **Pod request signing fails closed instead of silently going out unsigned** (ADR-2064). `[integrations.solid_pod_rs].sign_requests` was a best-effort hint: `_solid-http-base.js` bound `this._fetch = this._rawFetch` whenever the originator was falsy, and `_signedFetch` fell back to `this._rawFetch(...)` whenever no header could be built — and `lib/pod-signer.js` returns `null` for *both* "signing not wanted" and "signing wanted but unavailable", so an unresolvable stack or an undecryptable key emitted an unsigned request against a default-deny pod with nothing surfaced. Not hypothetical: the deployed manifest sets `sign_requests = true` while the supervised management-api exports no `AGENTBOX_STACK` and the manifest sets no `sign_stack`, so every pods request goes out unsigned today. The adapter now takes `requireSigned`, projected from that same manifest key by `adapters/index.js` and carried **even when the signer could not be built**; when a header cannot be produced it throws the typed `SigningUnavailable` (`code: SIGNING_UNAVAILABLE`, `slot: 'pods'`) and emits no request, so the slot degrades visibly per the adapter contract. A caller-supplied `Authorization` is still trusted and never overwritten, `sign_requests = false` stays byte-identical to the prior unsigned path, and there is **no dev-profile relaxation** — as established at ADR-2041, no `AGENTBOX_DEV*`/`dev_profile`/`dev_mode` flag exists in this codebase and this change does not invent one. Coverage asserts the guarantee per adapter class (`local-solid-rs`, `external`; `off` exempt) with a fetch spy proving **zero calls escaped**: 42 → 53 passing across the two suites, whole tree 80 suites / 1289 passing. Activation prerequisite recorded in ADR-2066: signing cannot currently succeed in this container at all, so leaving the key `true` now fails the slot rather than degrading it — provision the key material or set it `false` deliberately.
- **Exactly one process writes the pod inbox** (ADR-2065). The Rust `nostr-pod-bridge` daemon (`[program:nostr-relay]`, pid 388) and management-api's in-process JS `RelayConsumer` (pid 385) were **both** confirmed live and both writing `pods/<npub>/events/inbox/<id>.json`. The overlap was total rather than gift-wrap-only: Rust `process_event` applies a pubkey allowlist and an addressed-to check but **no kind filter**, keying the write on the same outer `ev.id` — the identical filename. Worse than duplication, it was suppression: the JS consumer deduplicated by testing whether the inbox file existed, the daemon shares the relay broadcast in-process and normally wins that race, so its write read as a duplicate and the JS consumer returned **before** dispatching the surfaces only it implements — ACSP governance (31400-31405, including the ADR-037 D2 waiter that releases an authority gate awaiting a signed 31403), agent-intent (38000+) and payments (38200/38201). Outright deletion was rejected: none of those, nor the outbox publisher and external fanout, exist in the Rust crate, and two test suites import the module. `RelayConsumer` now takes `writeInbox` (default **true**, so an external relay slot where the daemon does not run keeps its only writer); in delegated mode dedup is in-process rather than probing a file another process owns. Ownership is stated, not guessed: `flake.nix` projects `AGENTBOX_POD_INBOX_WRITER` from `podBridgeEnabled` — the same expression gating the daemon's supervisor block — and `server.js` reproduces that expression from env the image already exports until a rebuild carries it, so the two cannot disagree. `[sovereign_mesh.relay].pod_bridge` keeps its meaning and the daemon's block, env and behaviour are untouched. Verified divergence recorded rather than fixed: the JS consumer's `AGENTBOX_RELAY_ALLOWED_PUBKEYS` is never set by `flake.nix` and its `allowlist` policy treats an empty set as allow-all, so its configured `ingress_policy` is inert and it inherits whatever the relay admits — non-exploitable behind the ADR-2012 relay gate, but a defence-in-depth gap.

### Added (2026-09-05 briefing workflow routes, ADR-2072)

- **The briefing workflow VisionClaw already called now has a server.** `src/services/briefing_service.rs` drives a brief → execute → debrief cycle through `ManagementApiClient`, whose `create_brief`/`execute_brief`/`create_debrief` POST to `/v1/briefs`, `/v1/briefs/:brief_id/execute` and `/v1/briefs/:brief_id/debrief` — a grep of `management-api/` found no `/v1/briefs` registration anywhere, only a doc-comment mention at `middleware/linked-data/surfaces/s01-pods.js:10`, so every `BriefingService` call 404'd against a live, fully-typed client (VisionClaw ADR-2085 recorded the gap and chose implementation over client removal). New `management-api/routes/briefing.js`, registered in `server.js` beside the other adapter-backed surfaces, serves all three at 201/202/201. The contract is **derived, not designed**: request bodies stay snake_case as the Rust client literally writes them, response envelopes are camelCase (`briefId`, `briefPath`, `beadId`, `roleTasks`, `debriefPath`), and the `RoleTask` members nested inside `roleTasks` are snake_case (`task_id`, `bead_id`, `response_path`) because `crate::types::user_context::RoleTask` carries no `#[serde(rename_all)]` while its enclosing struct does — Fastify strips response properties absent from the schema, so getting that asymmetry wrong would empty the field on the client silently rather than loudly. Durability rides the slots (ADR-005): every document write goes through `fastify.adapters.pods` so the observability → privacy-filter → JSON-LD chain wraps it, and the route holds **no in-memory brief state** — the brief record is itself a pod resource at a path derived from the brief URN, so `execute`/`debrief` recover it by read and a management-api restart mid-cycle is survivable (asserted by rebuilding the app around the same store between calls). Work-ledger linkage through `fastify.adapters.beads` (epic on create, child per role, close on debrief) is best-effort: the slot resolving `off` degrades `beadId`/`bead_id` to null and never fails an otherwise-written brief, and the pods slot resolving `off` self-gates 503 on all three. Identifiers are minted via `lib/uris.js` (`kind: 'thing'`, optional owner scope from `user_context.pubkey`; an unusable pubkey mints the unscoped form rather than a fabricated scope). The execute step is gated **exactly** as `POST /v1/tasks` is (ADR-2041): each role spawn dispatches through `lib/action-plane.js` `dispatchTaskSpawn()` and fails closed with 503 when the plane has no live events adapter, 403 when the pipeline denies. `middleware/cost-gate.js` was deliberately **not** attached — its own first guard scopes it to URLs starting `/v1/tasks`, so on `/v1/briefs` it would be an inert decoration implying a spend check that never runs. New `tests/sovereign/briefing-route.test.js` (10 cases) covers the cycle, the snake_case members, restart survival, both gate paths and both degraded slots; whole tree 80 suites / 1289 passing against a 79/1279 baseline. VisionClaw side: 4 `#[cfg(test)]` cases in `briefing_service.rs` drive a real `ManagementApiClient` against a mock origin (no new Cargo dependency). Staged, not live — management-api is baked into the image, so the routes serve traffic after the next rebuild.

### Changed (2026-09-05 listener exposure gate, ADR-2062)

- **`scripts/ci/check-ports-loopback.sh` is now a listener gate as well as a publish gate.** A loopback compose publish constrains host→container traffic only; agentbox joins the external `visionclaw_network`, so a supervised program binding `0.0.0.0` inside the container is reachable, unauthenticated, by every sibling container — and is invisible to a `ports:`-based gate in any syntax, because it is never declared as a port. The gate now also parses the generated supervisord `command=` and `environment=` lines in `flake.nix` for bind addresses (`--bind`, `--bind-addr`, `--host`, `--listen`, `--ip`, `--address`, host-carrying `--port`, bare IPv4 positionals, and `*_BIND`/`*_HOST` env assignments), resolving Nix `${…}` interpolation from single-line `let` bindings and `cfg.attr or "default"` forms — and reporting an explicit **UNRESOLVED** verdict (exit 2) rather than passing where it cannot. A program binding non-loopback must be on the new `LISTENER_SANCTIONED` list, keyed by program name with a reason citing the governing record (seeded with `code-server` and `jupyter-lab`, both authenticated per ADR-2040), or it fails with the program, flag, address and `flake.nix` line. Every run prints the full listener inventory with each bind address, its verdict, and a `manifest-overridable` mark where the value came from a manifest default. **The publish rule is behaviourally and byte-for-byte unchanged** — other tooling parses its output. New fixtures `tests/security/check-listeners.test.mjs` (15 `node:test` cases) are wired into `.github/workflows/invariants.yml` beside the gate. First live run found a third instance of the class: `[program:wayvnc]` binds `0.0.0.0:5901` with no credential; `flake.nix` belongs to ADR-2040 so it was left untouched, the gate's exit code left honest, and the finding recorded in ADR-2062.

### Fixed (2026-09-05 recall gate, ADR-2018)

- **The recall gate failed its enforced floor (true 96/120) for a found cause:** the ruvector HNSW *parallel* index build (16 maintenance workers) leaves about a fifth of rows unreachable in the graph — reproduced by a parallel `REINDEX` (self 151/200), fixed by a serial one (self 189/200, true 115/120, PASS). The database is pinned to serial builds; the index law in CLAUDE.md and LEARNING-memory invariant 8 now say so.
- **`action-plane.js` compared the pipeline's `deny` against `denied`**, so a denial fell through to the allow branch and threw a 500 where the route meant 403 (latent; found by the governance-gaps lane). Fixed with a regression test.

### Fixed (2026-09-05 post-rebuild runtime, ADR-2063)

- **`agentbox-mcp-hub` went FATAL at first boot** (priority 205 started minutes before the entrypoint wrote `/run/agentbox/mcp-hub.json`), taking every hub-routed MCP server with it. The hub now waits for its config (`--wait-config-secs`, default 600) and the entrypoint starts or restarts it after each projection.
- **AoE session records died with every restart** (`~/.config` is a tmpfs) while their worktrees persisted: 18 `antigravity-N` + 18 `loom-N` checkouts had accumulated. A new `aoe-profiles` volume persists `profiles/`, and the seeder reaps its own orphans — clean, unreferenced, commit-free worktrees named exactly `<slug>` / `<slug>-N` — and moves a dead non-worktree directory aside (the `loom-raw` seed had failed every boot since 2026-08-31 on one). Regression-tested in `tests/cli/aoe-seed-orphans.test.mjs`.
- **The session seed `model` never reached `agy`** (AoE 1.13 drops `extra_args` for native agents); it now rides the per-agent override, so the antigravity session runs `agy --model gemini-3.8-flash`.
