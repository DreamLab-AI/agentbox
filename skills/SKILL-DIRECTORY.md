...
| `token-audit` | No | Comprehensive Claude Code usage audit from local transcripts (`~/.claude/projects/**/*.jsonl`): tokens by day/model/project, tool + MCP usage, subagent fan-out, cache efficiency, startup context tax, hourly activity, runaway-daemon cross-reference. Rust binary `token-audit` (ported from stdlib-only Python adapted MIT from pacphi/agentic-kit). Companions: `ruflo-daemon-gc` (pid-reuse-guarded reap), `npx-stale-scan.sh` | "Where are my tokens going", hitting plan limits, suspected runaway automation, usage breakdown requests |
