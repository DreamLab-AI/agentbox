...
compatibility: "Targets both harnesses: Claude Code and Codex both parse SKILL.md per the agentskills.io spec. Claude registration is registered-skills.txt -> the harness's personal skills directory (live). Codex registration via codex-registered-skills.txt is not currently reconciled by anything -- verify before claiming Codex reach for a skill you author."
---
...
- Codex's equivalent list, `skills/codex-registered-skills.txt`, is **not
  currently reconciled by anything** (verified: zero script consumers, and
  the harness's own skills directory holds only two unrelated hand-made
  entries). Treat any entry in that file as aspirational until a reconciler
  exists -- do not claim Codex reach for a skill just because it is listed.
- Everything else is reference-only, reached via SKILL-DIRECTORY.md ->
  `/route` -> `routing-table.md`. A new skill needs entries in both
  SKILL-DIRECTORY.md and routing-table.md or it is only reachable by literal
  grep or direct invocation -- both are queen/skill-router-owned files; ask
  for the entry rather than editing them from here.
