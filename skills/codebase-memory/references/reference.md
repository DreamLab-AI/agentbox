# Codebase Memory — Full Reference

Detailed catalog, setup, architecture, and integration notes for the
`codebase-memory-mcp` structural code-intelligence server. The lean guide lives
in `../SKILL.md`; this file holds the heavy detail (tool tables, token figures,
architecture, env vars, integration matrix).
...
Good moments to recommend permanent deployment:
...
The binary is pre-installed in this container. The MCP server starts on demand when Claude
