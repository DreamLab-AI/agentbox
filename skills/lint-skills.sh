...
SUPPRESS='DEAD|dead|retired|legacy|is not|never target|lint-ok|deprecated'
while IFS= read -r hit; do
  echo "STALE  $hit"; fail=1
done < <(grep -rInE "$BANNED" --include='SKILL.md' --include='*.md' --exclude-dir=toprank -- . | grep -vE "$SUPPRESS" | grep -v 'lint-skills')

# 2. Absolute ~/.claude/skills paths (skills are baked at /opt/agentbox/skills — use skill-relative paths).
while IFS= read -r hit; do
  echo "ABSPATH $hit"; fail=1
done < <(grep -rIn '~/.claude/skills/' --include='SKILL.md' --include='*.md' --exclude-dir=skill-builder --exclude-dir=architecture-studio --exclude-dir=toprank . | grep -vE '/opt/agentbox/skills|lint-ok')

# 3. Retired literal /workspace path (must be /home/devuser/workspace).
while IFS= read -r hit; do
  echo "RETIRED-PATH $hit"; fail=1
done < <(grep -rInE '(^|[^a-zA-Z0-9_./~-])/workspace/' --include='SKILL.md' --include='*.md' . | grep -v '/home/devuser/workspace' | grep -v '\$WORKSPACE')
