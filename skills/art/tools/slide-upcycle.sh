...
addendum() {
  case "$1" in
    04) echo 'DIAGRAM SLIDE — keep the architecture: a "Sovereign corpus (visionGraph + Whelk closure, 8,146 classes served)" box, an arrow to a central "One door — the vault CLI, budget-bounded, fail-open" box, which branches with arrows to two boxes: "PUSH — per-turn breadcrumb" and "PULL — subgraph on demand". Keep the two bullets: read-pervasive (every agent/turn can consult the KG) and write-governed (proposals auth-gated and queued; derived facts fenced).';;
