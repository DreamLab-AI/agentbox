...
researchAgents.forEach(agent => {
  mcp__claude-flow__agent_spawn({ agentType: agent.type, name: agent.name, capabilities: agent.capabilities })
})
```
...
devTeam.forEach(member => {
  mcp__claude-flow__agent_spawn({ agentType: member.type, name: member.name, capabilities: member.capabilities, swarmId: "dev-swarm" })
})
```
...
testingTeam.forEach(tester => {
  mcp__claude-flow__agent_spawn({ agentType: tester.type, name: tester.name, capabilities: tester.capabilities, swarmId: "testing-swarm" })
})
```
...
analysisTeam.forEach(analyst => {
  mcp__claude-flow__agent_spawn({ agentType: analyst.type, name: analyst.name, capabilities: analyst.capabilities })
})
```
