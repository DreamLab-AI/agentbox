...
`mcp.json`'s live qgis registration (queen-owned) instead sets `QGIS_HOST` /
`QGIS_PORT` / `QGIS_TIMEOUT` — names the FastMCP server above does not read,
so as configured they have no effect. See this skill's final report for the
