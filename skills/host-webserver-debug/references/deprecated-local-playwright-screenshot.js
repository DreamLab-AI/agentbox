#!/usr/bin/env node
/**
 * DEPRECATED — archived for reference only. Do NOT use.
 *
 * This drove a LOCAL Playwright/Chromium (the pre-sidecar path). Screenshots are now
 * taken through the browsercontainer sidecar via the `browser-gpu` MCP tools
 * (mcp__browser-gpu__navigate_page + take_screenshot). See ../SKILL.md and the `browser`
 * skill. Kept only so the historical implementation is not lost.
 *
