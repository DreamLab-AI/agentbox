#!/usr/bin/env python3
"""token-audit — a comprehensive picture of your Claude Code usage.

Adapted for agentbox from pacphi/agentic-kit (claude/skills/ruflo-token-audit,
MIT License, Copyright (c) 2026 Chris Phillipson). Changes: renamed, daemon
remediation hint points at /opt/agentbox/scripts/ruflo-daemon-gc.py.
...
Usage:
  token-audit.py                 # last 7 days, human report
  token-audit.py --days 30       # widen the window
  token-audit.py --json          # machine-readable (dashboards/CI)
  token-audit.py --top 20        # show more projects/sessions/tools
  token-audit.py --no-daemons    # skip the `ps` daemon cross-reference
"""
import argparse
import collections
...
def human_report(data, top, show_daemons):
    ...
    if show_daemons:
        ...
        if not daemons:
            ...
            w(f"\n  {len(daemons)} daemon(s) running. Inspect/stop: python3 /opt/agentbox/scripts/ruflo-daemon-gc.py [--kill]")
    return "\n".join(out)
