...
/**
 * EXP-AC-003 re-audit (auditor counter-example, commit 887679ff3).
 *
 * `derive()` fed a SKILL.md frontmatter block through `applyOperatorOverride`,
 * which SETS verifiability and stakes freely. But a SKILL.md ships with the
 * skill — it is not the operator's `agentbox.toml`. Treating it as an operator
 * surface let a skill file declare itself inspectable and bounded on an action
 * the operator had declared opaque and critical. Frontmatter now goes through the
 * SAME tightening lattice as an agent-supplied request triple: it may tighten any
 * axis, and loosen none.
 */
describe('EXP-AC-003 — a SKILL.md frontmatter triple may only TIGHTEN', () => {
  test('COUNTER-EXAMPLE: frontmatter cannot loosen verifiability or stakes', () => {
    const props = tp.derive('pod_delete', {
      manifest: MANIFEST,
      frontmatter: { task_properties: { verifiability: 'inspectable', stakes: 'bounded' } },
    });
    expect(props).toEqual({ verifiability: 'opaque', reversibility: 'irreversible', stakes: 'critical' });
  });

  test('a frontmatter triple that TIGHTENS is still honoured on every axis', () => {
    const props = tp.derive('memory_read', {
      manifest: MANIFEST,
      frontmatter: { task_properties: { verifiability: 'opaque', reversibility: 'irreversible', stakes: 'critical' } },
    });
    expect(props).toEqual({ verifiability: 'opaque', reversibility: 'irreversible', stakes: 'critical' });
  });

  test('COUNTER-EXAMPLE: frontmatter authority_class cannot loosen the manifest seed', () => {
    const props = tp.derive('pod_delete', {
      manifest: MANIFEST,
  ...
  test('frontmatter authority_class may still TIGHTEN an operator-recoverable class', () => {
    const props = tp.derive('memory_read', {
      manifest: MANIFEST,
      frontmatter: { authority_class: 'zero-tolerance' },
    });
  ...
  test('PROPERTY: no frontmatter triple, on any action class, is ever looser than the manifest derivation', () => {
    const axes = { verifiability: tp.VERIFIABILITY, reversibility: tp.REVERSIBILITY, stakes: tp.STAKES };
    for (const actionClass of ['pod_delete', 'memory_read', 'unclassified_thing']) {
      const base = tp.derive(actionClass, { manifest: MANIFEST });
      for (const v of tp.VERIFIABILITY) {
        for (const r of tp.REVERSIBILITY) {
          for (const s of tp.STAKES) {
            const got = tp.derive(actionClass, {
              manifest: MANIFEST,
              frontmatter: { task_properties: { verifiability: v, reversibility: r, stakes: s } },
            });
            for (const axis of Object.keys(axes)) {
              expect(tp.rank(axis, got[axis])).toBeGreaterThanOrEqual(tp.rank(axis, base[axis]));
            }
          }
        }
      }
  ...
  test('derive threads a logger through to classifyAction so a refused loosening is reported', () => {
    const warnings = [];
    tp.derive('pod_delete', {
      manifest: MANIFEST,
      frontmatter: { authority_class: 'recoverable' },
