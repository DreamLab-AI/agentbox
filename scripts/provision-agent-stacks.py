...
# Image-baked SessionEnd hook that mirrors a curated digest to the operator's
# phone via the Nostr mobile bridge (gated on [sovereign_mesh.mobile_bridge]).
...
def learning_hooks(mobile_bridge: bool = False) -> dict:
    ...
    When ``mobile_bridge`` is set, a second SessionEnd hook mirrors a curated
    Z.AI digest to the operator's phone via ``nostr-session-summary.py`` (the
    Nostr replacement for the retired Telegram mirror). It runs after the
    learning hook; its longer timeout covers the Z.AI summarisation round-trip.
    """
    ...
    session_end = [{"hooks": [cmd("session-end", 10000)]}]
    if mobile_bridge:
        session_end.append(
            {
                "hooks": [
...
def main() -> None:
    ...
    # SessionEnd phone mirror is registered only when the mobile bridge is on.
