...
section_consultants() {
  ...
  raw="$(wt_checklist "Consultant tier — providers to enable" \
    ...
    "consultants.zai.enabled"        "Z.AI / GLM-5.2 via claude-zai (providers.zai)"        "$([[ "$(state_get consultants.zai.enabled)" == "true" ]] && echo ON || echo OFF)" \
