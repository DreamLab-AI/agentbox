...
//! reasoning:
//!
//! - **`38414` Graduation is refused.** A graduation is the *record of a human
...
pub const SIGNABLE_KINDS: &[u64] = &[
    38_410, // KnowledgeUnit
    38_411, // Confirmation
    38_412, // Flag
    38_413, // Supersession
    38_415, // ToolGapSignal
];

/// Graduation. Refused on purpose — see the module docs.
pub const KIND_GRADUATION: u64 = 38_414;
...
pub enum Refusal {
    ...
    #[error(
        "kind 38414 (Graduation) is refused: it records a human decision, and signing it with the \
...
mod tests {
    ...
    fn every_colloquy_kind_except_graduation_is_admitted() {
        for kind in [38_410, 38_411, 38_412, 38_413, 38_415] {
    ...
    fn a_request_cannot_name_its_author() {
        ...
        let req = parse_request(
            r#"{"pubkey":"deadbeef","kind":38410,"tags":[["d","abc"]],"content":"{}"}"#,
        )
        .unwrap();
        assert_eq!(req.kind, 38_410);
    ...
    fn created_at_is_optional_and_tags_default_to_empty() {
        let req = parse_request(r#"{"kind":38411}"#).unwrap();
