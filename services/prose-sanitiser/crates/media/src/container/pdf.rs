...
/// Clean a PDF's provenance metadata.
///
/// Prefers the object-graph rewrite. A file `lopdf` cannot parse falls back to
/// deleting XMP packets in place, which is clearly reported as degraded: it
/// cannot reach `/Info`, and it leaves the cross-reference offsets stale.
///
/// # Errors
///
/// Returns `Err` when the input cannot be read or the output cannot be written.
pub fn clean_pdf(path: &Path, dest: &Path) -> Result<(Vec<String>, Value), String> {
    ...
    match Document::load_mem(&data) {
        Ok(mut doc) => {
            ...
            return Ok((
                actions,
                json!({"mode": "lopdf", "structural_rewrite": true}),
            ));
        }
        Err(error) => actions.push(format!("lopdf could not parse the document: {error}")),
    }

    // Degraded: strip obvious XMP packets from the raw bytes.
    ...
    if count > 0 {
        ...
        safe_write_bytes(dest, &stripped)
            .map_err(|e| format!("cannot write {}: {e}", dest.display()))?;
        actions.push(
            "warning: the byte-level fallback cannot reach /Info and does not rewrite the object \
             graph, so earlier revisions may remain recoverable"
                .to_string(),
        );
        return Ok((
            actions,
            json!({"mode": "stdlib-xmp", "degraded": true, "structural_rewrite": false}),
        ));
    }

    safe_write_bytes(dest, &data).map_err(|e| format!("cannot write {}: {e}", dest.display()))?;
    actions.push("no PDF metadata found by the byte-level fallback; copied as-is".to_string());
    Ok((
        actions,
        json!({"mode": "copy", "degraded": true, "structural_rewrite": false}),
    ))
}
