...
- **memory + learning surfaces are manifest-gated** ([PRD-018](docs/reference/prd/PRD-018-ruvector-native-memory-and-learning.md) / [ADR-036](docs/reference/adr/ADR-036-ruvector-capability-adoption-and-learning-loop.md) / [DDD-016](docs/reference/ddd/DDD-016-memory-learning-domain.md)) and, as of 2026-07-05, **live in this deployment's manifest** — check `agentbox.toml` directly, it is the running configuration, not a template. All six `[integrations.ruvector_external]` retrieval gates (`hybrid_search`, `typed_metadata`, `metadata_gin`, `health_tool`, `episodic_ttl_sweep`, `memory_orient`) are `true`, so hybrid search, typed metadata/TTL, and `memory_health`/`memory_orient` are active. `[memory_learning].enabled` + `record_trajectories` are `true` (the producer is live: `config/hooks/trajectory-recorder.cjs` records real trajectories), but the two consumers — `feed_retrieval` and `feed_routing` — stay `false` (corpus-gated) until the trajectory corpus clears the Wilson floor (`aggregate_min_samples = 20`); `sona_enabled`/`relevance_feedback` remain reserved ADOPT-LATER. `[memory_hygiene]`'s three ops (`allow_namespace_repair`, `allow_embedding_backfill`, `allow_legacy_archival`) are back to `false` (fail-closed, re-sealed) — each one ran its non-dry-run pass exactly once on 2026-07-05 (see CHANGELOG) and the gate was then closed again. Sidecar lifecycle (pin-by-digest updates, snapshot-rehearsed; hygiene ops dry-run unless their gate is open): `./agentbox.sh ruvector <status|check|test|update|rollback|…>`.
