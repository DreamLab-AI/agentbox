...
- Permission mode is **`[claude_code].permission_mode`** (ADR-2116; default `bypassPermissions`, no classifier), projected into `~/.claude` and every profile's settings at boot — edit `agentbox.toml`, not `settings.json` (Claude Code rewrites it from memory). Deny rules (`docker run`/`compose`, `ssh`) hold even in bypass but are prefix matches. `dsp` = manifest mode, `dspa` = auto, `dspb` = bypass. On 2.1.280 bypass also allows `.git/`/`.claude/` writes. Boot seeds folder trust (`config/hooks/trust-seed.cjs`).
