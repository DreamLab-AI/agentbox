...
- `[vault]` in [`agentbox.toml`](agentbox.toml): the single path authority for the authored corpus (ADR-2028) — the entrypoint exports `VAULT_ROOT`/`VAULT_PAGES`/`VAULT_FORMAT`/`VAULT_TUI` to every supervised program, tmux window and MCP server, no consumer hard-codes a corpus path, and `tui = "rune"` opens the vault in tmux window 9 **"Notes"** (ADR-2029); writers emit V2 frontmatter via [`mcp/servers/lib/vault-frontmatter.js`](mcp/servers/lib/vault-frontmatter.js)
...
- [`crates/colloquy/`](crates/colloquy/): the cq shared-agent-learning model, clean-room in Rust (ADR-2085/2086). Six crates: `colloquy-core` (the standard — pure, wasm-capable, Apache-2.0, publishable), `colloquy-view` (presentation models; depends on core alone, which is what keeps the forum from path-depending this repo), `colloquy-nostr` (kinds 38100–38105), `colloquy-store` (local / shared / relay behind one trait, **no sixth adapter slot**), `colloquy-backends` (the production transports: the governed `ruvector-mcp.cjs` as a child process, and a websocket to the relay), and `colloquy-mcp` (the six verbs — `query`/`propose`/`confirm`/`flag`/`reflect`/`status` — over stdio, tier chosen by `COLLOQUY_TIER`). Confidence counts **authorising principals**, never accounts: an operator's fifty agents are one voice, and an unregistered pubkey is dropped rather than self-authorising. It supersedes `precedent-service.js`/`precedent-bridge.js`, which are annotated and scheduled for deletion behind their own migration
...
- AoE session records persist on the `aoe-profiles` volume and the seeder reaps only its own clean, unreferenced orphan worktrees (ADR-2063); `agentbox-mcp hub` waits for `/run/agentbox/mcp-hub.json` rather than crashing FATAL on the boot priority race. If ten hub-routed MCP servers refuse connections, `supervisorctl status agentbox-mcp-hub` first.
