...
- [`DreamLab-AI/sidestr-rs`](https://github.com/DreamLab-AI/sidestr-rs) (**not in this repo** — ADR-2112): sidestr-rs — Rust port of Melvin Carvalho's sidestr sidechains, AGPL-3.0-only: the economic engine for did:nostr agents. A did:nostr key is a sidechain wallet. Five crates **published** on crates.io (PRD-024, ADR-2096/2106) — [`sidestr-header`](https://crates.io/crates/sidestr-header) (both header families, no_std, RustCrypto only), [`sidestr-core`](https://crates.io/crates/sidestr-core) (chain document, block build/sign, rules, block file, validating chain), [`sidestr-nostr`](https://crates.io/crates/sidestr-nostr) (own NIP-01 event, sealed `SignRequest`, the sidestr kinds and the estate's 38420-38425), [`sidestr-wallet`](https://crates.io/crates/sidestr-wallet) (coins, selection, key-path spends, burns, peg-in shape) and [`sidestr-round`](https://crates.io/crates/sidestr-round) (level-2 consensus round and the `cosign` binary) — all **`AGPL-3.0-only`** attributed ports of `siding` (plus the schema kernel and blaketestnode), never dual-licensed, never a dependency of a permissive crate, proven against the JS reference as oracle, and audited anti-fox before every publish (GPT-6 Astra via the codex CLI, neutral verification-engineering wording — adversarial wording trips the content filter; findings become `tests/audit_regressions*.rs` there). Source, CI and releases live in that repo; any estate consumer takes them from crates.io. `crates/sidestr/README.md` is only a pointer. What agentbox hosts is the **chain instance** — the sealed root chain document, interim producer runner and mirror sync: [`config/sidechain/`](config/sidechain/). Testnet only; no real funds.
