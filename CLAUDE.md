...
- **memory + learning surfaces are manifest-gated, default OFF** ([PRD-018](docs/reference/prd/PRD-018-ruvector-native-memory-and-learning.md) / [ADR-036](docs/reference/adr/ADR-036-ruvector-capability-adoption-and-learning-loop.md) / [DDD-016](docs/reference/ddd/DDD-016-memory-learning-domain.md)): hybrid search, typed metadata/TTL, `memory_orient`, `memory_health`, the trajectory-recording learning loop, and the data-hygiene ops all sit behind `[integrations.ruvector_external]` / `[memory_learning]` / `[memory_hygiene]` flags. Sidecar lifecycle (pin-by-digest updates, snapshot-rehearsed; hygiene ops dry-run by default): `./agentbox.sh ruvector <status|check|test|update|rollback|…>`.
