...
- **Adapter middleware is cross-cutting.** Observability (ADR-005) and the privacy filter (ADR-008) are the two middleware layers that wrap every adapter dispatch. New cross-cutting concerns follow the same shape: one hook point, one policy per slot, fail-closed/fail-open semantics explicit in the ADR.
